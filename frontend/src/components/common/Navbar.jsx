...
const NOTIF_STYLE = {
  ...
  RIDE:    { icon: Car,       color: 'text-indigo-600 bg-indigo-50' },
  PAYMENT: { icon: Wallet,    color: 'text-emerald-600 bg-emerald-50' },
  RATING:  { icon: Star,      color: 'text-amber-600 bg-amber-50'   },
  GENERAL: { icon: Bell,      color: 'text-gray-600 bg-gray-50'     },
};
...
function NotificationBell({ count, onClick, isOpen }) {
  return (
    ...
    >
      <Bell className={`h-5 w-5 ${isOpen ? 'text-indigo-600' : 'text-gray-500'}`} />
      {count > 0 && (
...
function NotificationsDropdown({ isOpen, onClose }) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl border border-gray-100
      shadow-xl shadow-gray-500/10 z-50 animate-fadeIn overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs font-semibold text-indigo-600
            hover:text-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed"
        >
            ...
            return (
              <div
                ...
                className={`flex gap-3 px-4 py-3 border-b border-gray-50 last:border-0 cursor-pointer
                  hover:bg-gray-50 transition-colors ${n.read ? '' : 'bg-indigo-50/40'}`}
              >
      ...
      <div className="px-4 py-2.5 border-t border-gray-100">
        <Link
          to="/notifications"
          onClick={onClose}
          className="block text-center text-xs font-semibold text-indigo-600 hover:text-indigo-700 py-1"
        >
...
function UserDropdown({ isOpen, onClose, user, isAdmin, isDriver, dashLink, handleLogout, isOnboarding }) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-60 bg-white rounded-2xl border border-gray-100
      shadow-xl shadow-gray-500/10 z-50 animate-fadeIn overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2 mt-3">
          {isAdmin ? (
            <span className="badge-purple flex items-center gap-1 text-[10px] px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full border border-purple-200">
              <Shield className="h-2.5 w-2.5" /> Admin
            </span>
          ) : isDriver ? (
            <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full border border-blue-200">
              <Car className="h-2.5 w-2.5" /> Driver
            </span>
          ) : (
            <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-200">
      ...
      {/* Logout */}
      <div className={`p-2 ${menuItems.length > 0 ? 'border-t border-gray-100' : ''}`}>
        <button
...
function MobileBottomNav({ isAdmin, isDriver, isPassenger, dashLink, isOnboarding }) {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200
      px-2 py-1.5 flex items-center justify-around z-50 shadow-lg">
...
export default function Navbar() {
  ...
  // Drivers who have not completed verification are locked to a minimal
  // navbar (logo + logout only) on both the document-upload flow and the
  // post-submission waiting page. They must not see dashboard, ride, or
  // notification features until an admin approves their account.
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-xl font-bold text-indigo-600">Wave</span>
              </div>
            </div>
  ...
  return (
    <>
      <nav className={`sticky top-0 z-40 transition-all duration-300 ${
        scrolled
          ? 'bg-white/90 backdrop-blur-lg border-b border-gray-200/50 shadow-lg shadow-gray-500/5'
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              {user ? (
                ...
                <div className="flex items-center gap-2">
                  <Link to="/login"    className="px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-xl transition-colors">Login</Link>
                  <Link to="/register" className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-lg shadow-indigo-500/25 transition-all">Sign Up</Link>
                </div>
              )}
            </div>
          ...
          <div className="md:hidden border-t border-gray-100 bg-white animate-fadeIn">
            <div className="px-4 py-4 space-y-1 max-h-[calc(100vh-8rem)] overflow-y-auto">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-gray-500">{isDriver ? 'Driver' : isAdmin ? 'Admin' : 'Passenger'}</span>
                      {!isAdmin && (
              ...
              <NavLink to="/" end onClick={() => setMobileOpen(false)}
                className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${isActive ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-50'}`}>
              ...
              {user ? (
                <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50 transition-colors w-full mt-2"
                  >
                ...
                <div className="flex gap-3 pt-3">
                  <Link to="/login"    onClick={() => setMobileOpen(false)} className="flex-1 px-4 py-3 text-sm font-semibold text-center text-gray-700 bg-gray-100 rounded-xl">Login</Link>
                  <Link to="/register" onClick={() => setMobileOpen(false)} className="flex-1 px-4 py-3 text-sm font-bold text-center text-white bg-indigo-600 rounded-xl">Sign Up</Link>
                </div>
              )}
            </div>
          </div>
